...
static ask::Screen::Mode mode;
static ask::FrameBuffer *fb;

static size_t activeWindow = WINDOW_COUNT;
static size_t topWindow = WINDOW_COUNT;
static Window windows[WINDOW_COUNT];

int win_init(int sid,ask::UI *uiobj,gsize_t width,gsize_t height,gcoldepth_t bpp,const char *shmname) {
	drvId = sid;
	ui = uiobj;
	srand(time(NULL));
...
static void win_createBuf(Window *win,gwinid_t id,gsize_t width,gsize_t height,const char *winmng) {
	...
	ask::Screen::Mode winMode = mode;
	winMode.guiHeaderSize = winMode.tuiHeaderSize = 0;
	winMode.width = width;
	winMode.height = height;
	win->fb = new ask::FrameBuffer(winMode,name,ask::Screen::MODE_TYPE_GUI,0666);
...
int win_setMode(gsize_t width,gsize_t height,gcoldepth_t bpp,const char *shmname) {
	...
	ask::Screen::Mode newmode = ui->findGraphicsMode(width,height,bpp);
	...
	try {
		...
		std::unique_ptr<ask::FrameBuffer> newfb(
			new ask::FrameBuffer(newmode,shmname,ask::Screen::MODE_TYPE_GUI,0644));
		print("Setting mode %d: %zux%zux%u",newmode.id,newmode.width,newmode.height,newmode.bitsPerPixel);
		ui->setMode(ask::Screen::MODE_TYPE_GUI,newmode.id,shmname,true);
		...
		for(size_t i = 0; i < WINDOW_COUNT; i++) {
			if(windows[i].id != WINID_UNUSED) {
				...
				ev.type = ask::WinMngEvents::Event::TYPE_RESET;
				ev.wid = windows[i].id;
		...
		fb = new ask::FrameBuffer(mode,shmname,ask::Screen::MODE_TYPE_GUI,0644);
		ui->setMode(ask::Screen::MODE_TYPE_GUI,mode.id,shmname,true);
...
static void win_sendActive(gwinid_t id,bool isActive,gpos_t mouseX,gpos_t mouseY) {
	...
	ask::WinMngEvents::Event ev;
	ev.type = ask::WinMngEvents::Event::TYPE_SET_ACTIVE;
	ev.wid = id;
...
static void win_notifyWinCreate(gwinid_t id,const char *title) {
	...
	ask::WinMngEvents::Event ev;
	ev.type = ask::WinMngEvents::Event::TYPE_CREATED;
	ev.wid = id;
...
static void win_notifyWinActive(gwinid_t id) {
	...
	ask::WinMngEvents::Event ev;
	ev.type = ask::WinMngEvents::Event::TYPE_ACTIVE;
	ev.wid = id;
	listener_notify(&ev);
}
...
static void win_notifyWinDestroy(gwinid_t id) {
	...
	ask::WinMngEvents::Event ev;
	ev.type = ask::WinMngEvents::Event::TYPE_DESTROYED;
	ev.wid = id;
	listener_notify(&ev);
}
