...
static void handleKbMessage(ask::UIEvents::Event *data);
static void handleMouseMessage(ask::WinMng &winmng,ask::UIEvents::Event *data);

static uchar buttons = 0;
static gpos_t curX = 0;
static gpos_t curY = 0;
static uchar cursor = ask::Screen::CURSOR_DEFAULT;
...
int input_thread(void *arg) {
	...
	while(1) {
		...
		*in->uiev >> ev;
		...
		switch(ev.type) {
			case ask::UIEvents::Event::TYPE_KEYBOARD:
				handleKbMessage(&ev);
				break;

			case ask::UIEvents::Event::TYPE_MOUSE:
...
static void handleKbMessage(ask::UIEvents::Event *data) {
	...
	ev.type = ask::WinMngEvents::Event::TYPE_KEYBOARD;
	ev.wid = active->id;
...
static void handleMouseMessage(ask::WinMng &winmng,ask::UIEvents::Event *data) {
	...
	if(!buttons) {
		...
		cursor = ask::Screen::CURSOR_DEFAULT;
		if(w && w->style != WIN_STYLE_POPUP && w->style != WIN_STYLE_DESKTOP) {
			gsize_t tbh = w->titleBarHeight;
			bool left = curY >= (gpos_t)(w->y() + tbh) &&
					curX < (gpos_t)(w->x() + ask::Screen::CURSOR_RESIZE_WIDTH);
			bool right = curY >= (gpos_t)(w->y() + tbh) &&
					curX >= (gpos_t)(w->x() + w->width() - ask::Screen::CURSOR_RESIZE_WIDTH);
			bool bottom = curY >= (gpos_t)(w->y() + w->height() - ask::Screen::CURSOR_RESIZE_WIDTH);
			if(left && bottom)
				cursor = ask::Screen::CURSOR_RESIZE_BL;
			else if(left)
				cursor = ask::Screen::CURSOR_RESIZE_L;
			if(right && bottom)
				cursor = ask::Screen::CURSOR_RESIZE_BR;
			else if(right)
				cursor = ask::Screen::CURSOR_RESIZE_R;
			else if(bottom && !left)
				cursor = ask::Screen::CURSOR_RESIZE_VERT;
		}
	}
	...
	if(w && w->evfd != -1) {
		ask::WinMngEvents::Event ev;
		ev.type = ask::WinMngEvents::Event::TYPE_MOUSE;
		ev.wid = w->id;
		ev.d.mouse.x = curX;
		ev.d.mouse.y = curY;
