...
static int mouseClientThread(A_UNUSED void *arg) {
	...
	/* open mouse */
	ask::IPCStream ms("/dev/mouse");
	ask::Mouse::Event ev;
	while(1) {
		ms >> ask::ReceiveData(&ev,sizeof(ev));

		ask::UIEvents::Event uiev;
		uiev.type = ask::UIEvents::Event::TYPE_MOUSE;
		uiev.d.mouse = ev;
...
static bool handleKey(ask::UIEvents::Event *data) {
...
static int kbClientThread(A_UNUSED void *arg) {
	/* open keyboard */
	ask::IPCStream kb("/dev/keyb");
	ask::Keyb::Event ev;
	while(1) {
		kb >> ask::ReceiveData(&ev,sizeof(ev));

		/* translate keycode */
		ask::UIEvents::Event uiev;
		uiev.type = ask::UIEvents::Event::TYPE_KEYBOARD;
