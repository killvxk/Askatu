...
const char *Header::OS_TITLE = "Askatu v" ASKATU_VERSION;
...
void Header::doUpdate(UIClient *cli,gsize_t width,gsize_t height) {
	if(cli->type() == ask::Screen::MODE_TYPE_TUI || width == cli->fb()->mode().width)
...
void Header::putcTUI(const ask::Screen::Mode &,char *header,uint *col,char c,char color) {
	header[*col * 2] = c;
	header[*col * 2 + 1] = color;
	(*col)++;
}

void Header::putcGUI(const ask::Screen::Mode &mode,char *header,uint *col,char c,char color) {
#if defined(__x86__)
...
void Header::makeDirty(UIClient *cli,size_t cols,gsize_t *width,gsize_t *height) {
	if(cli->type() == ask::Screen::MODE_TYPE_TUI) {
		*width = cols;
		*height = 1;
	}
	else {
...
void Header::buildTitle(UIClient *cli,gsize_t *width,gsize_t *height,bool force) {
	...
	uint col = 0;
	putc_func func = cli->type() == ask::Screen::MODE_TYPE_GUI ? putcGUI : putcTUI;
	const ask::Screen::Mode &mode = cli->fb()->mode();
