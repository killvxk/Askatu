...
class Header {
	...
	typedef void (*putc_func)(const ask::Screen::Mode &mode,char *header,uint *col,char c,char color);
...
public:
	...
	static size_t getHeight(int type) {
		if(type == ask::Screen::MODE_TYPE_TUI)
			return 1;
		return FONT_HEIGHT;
	}
	 ...
	 */
	static size_t getSize(const ask::Screen::Mode &mode,int type,gpos_t x) {
		if(type == ask::Screen::MODE_TYPE_TUI)
			return x * 2;
...
private:
	static void putcTUI(const ask::Screen::Mode &,char *header,uint *col,char c,char color);
	static void putcGUI(const ask::Screen::Mode &mode,char *header,uint *col,char c,char color);
