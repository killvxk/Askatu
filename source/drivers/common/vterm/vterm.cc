...
static ask::FrameBuffer *fb = NULL;
static sVTerm vterm;
static bool run = true;
static TUIVTermDevice *vtdev;

class TUIVTermDevice : public ask::VTermDevice {
public:
	...
	void getMode(ask::IPCStream &is) {
		ask::Screen::Mode mode = vterm.ui->getMode();
		is << 0 << mode << ask::Reply();
	}
	...
	void getModes(ask::IPCStream &is) {
		...
		is << modes.size() << ask::Reply();
		if(n)
			is << ask::ReplyData(modes.begin(),sizeof(ask::Screen::Mode) * modes.size());
	}

	void getKeymap(ask::IPCStream &is) {
		std::string keymap = vterm.ui->getKeymap();
		is << 0 << ask::CString(keymap.c_str(),keymap.length()) << ask::Reply();
	}
...
static int uimInputThread(void *arg) {
	...
	while(run) {
		...
		uiev.is() >> ask::ReceiveData(&ev,sizeof(ev),false);
		if(!run)
			break;

		if(ev.type == ask::UIEvents::Event::TYPE_KEYBOARD) {
			std::lock_guard<std::mutex> guard(*vterm.mutex);
...
static int vtInit(int id,const char *name,uint cols,uint rows) {
	vterm.ui = new ask::UI("/dev/uimng");
	modes = vterm.ui->getModes();

	/* find a suitable mode */
	ask::Screen::Mode mode = vterm.ui->findTextModeIn(modes,cols,rows);

	/* open speaker */
	struct stat info;
	if(stat("/dev/speaker",&info) >= 0) {
		try {
			vterm.speaker = new ask::Speaker("/dev/speaker");
		}
...
static void vtSetVideoMode(int mode) {
	for(auto it = modes.begin(); it != modes.end(); ++it) {
		if(it->id == mode) {
			...
			try {
				std::unique_ptr<ask::FrameBuffer> nfb(
					new ask::FrameBuffer(*it,vterm.name,ask::Screen::MODE_TYPE_TUI,0644));
				vterm.ui->setMode(ask::Screen::MODE_TYPE_TUI,it->id,vterm.name,true);
				fb = nfb.release();
			}
			catch(const ask::default_error &e) {
				fb = fbtmp;
				if(fb)
			...
			if(vterm.cols != it->cols || vterm.rows != it->rows) {
				if(!vtctrl_resize(&vterm,it->cols,it->rows)) {
					...
					if(fb) {
						fb->rename(vterm.name);
						vterm.ui->setMode(ask::Screen::MODE_TYPE_TUI,fb->mode().id,vterm.name,true);
					}
