...
private:
	static ask::Socket getEndpoint(CtrlConRef &ctrlRef) {
		...
		ask::Socket sock("/dev/socket",ask::Socket::SOCK_STREAM,ask::Socket::PROTO_TCP);
		ask::Socket::Addr addr;
		addr.family = ask::Socket::AF_INET;
		addr.d.ipv4.addr = ask::Net::IPv4Addr(parts[0],parts[1],parts[2],parts[3]).value();
