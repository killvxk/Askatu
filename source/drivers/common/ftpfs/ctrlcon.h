...
class CtrlCon {
	...
	explicit CtrlCon(const ask::Net::IPv4Addr &ip,ask::port_t port,
...
public:
	...
	explicit CtrlCon(const std::string &host,ask::port_t port,
			const std::string &user,const std::string &pw,const std::string &dir)
		: _refs(1), _available(true), _dest(ask::DNS::getHost(host.c_str())), _port(port),
