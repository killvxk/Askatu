...
void Ext2BGMng::update() {
	...
	/* update main block-group-daskriptor-table */
	if(Ext2RW::writeBlocks(_fs,_groups,le32tocpu(_fs->sb.get()->firstDataBlock) + 1,bcount) != 0) {
		printe("Unable to update block-group-daskriptor-table in blockgroup 0");
		goto done;
	}

	/* update block-group-daskriptor backups */
	...
	for(i = 1; i < count; i++) {
		if(_fs->bgHasBackups(i)) {
			if(Ext2RW::writeBlocks(_fs,_groups,bno,bcount) != 0) {
				printe("Unable to update block-group-daskriptor-table in blockgroup %d",i);
				goto done;
			}
		}
