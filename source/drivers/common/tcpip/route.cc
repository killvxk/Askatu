...
int Route::insert(const ask::Net::IPv4Addr &dest,const ask::Net::IPv4Addr &nm,
		const ask::Net::IPv4Addr &gw,uint flags,const std::shared_ptr<Link> &l) {
	// if we should use the gateway, it has to be a valid host
	if((flags & ask::Net::FL_USE_GW) && (gw.value() == 0 || !gw.isHost(nm)))
		return -EINVAL;
	if(!nm.isNetmask() || !l)
		return -EINVAL;
...
Route Route::find(const ask::Net::IPv4Addr &ip) {
	std::lock_guard<std::mutex> guard(_mutex);
	for(auto it = _table.begin(); it != _table.end(); ++it) {
		if(((*it)->flags & ask::Net::FL_UP) && (*it)->dest.sameNetwork(ip,(*it)->netmask))
			return **it;
	}
	return Route();
}

int Route::setStatus(const ask::Net::IPv4Addr &ip,ask::Net::Status status) {
	std::lock_guard<std::mutex> guard(_mutex);
	for(auto it = _table.begin(); it != _table.end(); ++it) {
		if((*it)->dest == ip) {
			if(status == ask::Net::DOWN)
				(*it)->flags &= ~ask::Net::FL_UP;
			else
				(*it)->flags |= ask::Net::FL_UP;
			return 0;
		}
...
int Route::remove(const ask::Net::IPv4Addr &ip) {
	std::lock_guard<std::mutex> guard(_mutex);
	for(auto it = _table.begin(); it != _table.end(); ++it) {
		if((*it)->dest == ip) {
			_table.erase(it);
