...
class ARP {
	...
	typedef std::map<ask::Net::IPv4Addr,ask::NIC::MAC> cache_type;
...
public:
	...
	static ssize_t requestMAC(const std::shared_ptr<Link> &link,const ask::Net::IPv4Addr &ip);
	static void print(std::ostream &os);

private:
	static int createPending(const void *packet,size_t size,
		const ask::Net::IPv4Addr &ip,uint16_t type);
...
public:
	...
	ask::NIC::MAC hwSender;
	ask::Net::IPv4Addr ipSender;
	ask::NIC::MAC hwTarget;
	ask::Net::IPv4Addr ipTarget;
