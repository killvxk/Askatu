...
int ARP::createPending(const void *packet,size_t size,const ask::Net::IPv4Addr &ip,uint16_t type) {
	PendingPacket pkt;
	pkt.dest = ip;
	pkt.size = size;
	pkt.type = type;
...
ssize_t ARP::requestMAC(const std::shared_ptr<Link> &link,const ask::Net::IPv4Addr &ip) {
	Ethernet<ARP> pkt;
	ARP *arp = &pkt.payload;

	arp->hwAddrFmt = cputobe16(HW_ADDR_ETHER);
	arp->hwAddrSize = sizeof(ask::NIC::MAC);
	arp->protoAddrFmt = cputobe16(IPv4<>::ETHER_TYPE);
	arp->protoAddrSize = ask::Net::IPv4Addr::LEN;
	arp->cmd = cputobe16(CMD_REQUEST);

	arp->hwTarget = ask::NIC::MAC();
	arp->ipTarget = ip;
	arp->hwSender = link->mac();
	arp->ipSender = link->ip();

	return Ethernet<ARP>::send(link,ask::NIC::MAC::broadcast(),&pkt,pkt.size(),ARP::ETHER_TYPE);
}
...
ssize_t ARP::handleRequest(const std::shared_ptr<Link> &link,const ARP *packet) {
	...
	if(packet->hwSender == ask::NIC::MAC::broadcast())
		return -EINVAL;
...
ssize_t ARP::send(const std::shared_ptr<Link> &link,Ethernet<> *packet,size_t size,
		const ask::Net::IPv4Addr &ip,const ask::Net::IPv4Addr &nm,uint16_t type) {
	ask::NIC::MAC mac;
	if(ip == ip.getBroadcast(nm))
		mac = ask::NIC::MAC::broadcast();
