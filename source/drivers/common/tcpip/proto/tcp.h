...
public:
	...
	static ssize_t send(const ask::Net::IPv4Addr &ip,ask::port_t srcp,ask::port_t dstp,uint8_t flags,
...
private:
	static ssize_t sendWith(Ethernet<IPv4<TCP>> *pkt,const ask::Net::IPv4Addr &ip,ask::port_t srcp,
		ask::port_t dstp,uint8_t flags,const void *data,size_t nbytes,size_t optSize,uint32_t seqNo,
		uint32_t ackNo,uint16_t winSize);

	static uint32_t getKey(ask::port_t localPort,ask::port_t remotePort) {
		return ((uint32_t)localPort << 16) | remotePort;
	}
	static ssize_t addSocket(StreamSocket *sock,ask::port_t localPort,ask::port_t remotePort) {
	...
	static void remSocket(StreamSocket *sock,ask::port_t localPort,ask::port_t remotePort) {
		uint32_t key = getKey(localPort,remotePort);
		socket_map::iterator it = _socks.find(key);
