...
class UDP {
	...
	typedef std::map<ask::port_t,DGramSocket*> socket_map;
...
public:
	...
	static ssize_t send(const ask::Net::IPv4Addr &ip,ask::port_t srcp,ask::port_t dstp,
...
private:
	static ssize_t addSocket(DGramSocket *sock,ask::port_t port) {
	...
	static void remSocket(DGramSocket *sock,ask::port_t port) {
		socket_map::iterator it = _socks.find(port);
