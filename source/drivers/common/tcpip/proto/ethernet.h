...
public:
	...
	static ssize_t send(const std::shared_ptr<Link> &link,const ask::NIC::MAC &dest,Ethernet<T> *pkt,
	...
	static ssize_t receive(const std::shared_ptr<Link> &link,const Packet &packet) {
		...
		for(auto it = RawEtherSocket::sockets.begin(); it != RawEtherSocket::sockets.end(); ++it) {
			if((*it)->protocol() == ask::Socket::PROTO_ANY || (*it)->protocol() == epkt->type)
				(*it)->push(ask::Socket::Addr(),packet);
		}
