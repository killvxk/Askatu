...
public:
	...
	static ssize_t send(Ethernet<IPv4<T>> *pkt,size_t sz,
			const ask::Net::IPv4Addr &ip,uint8_t protocol) {
	...
	static ssize_t receive(const std::shared_ptr<Link> &link,const Packet &packet) {
		...
		for(auto it = RawIPSocket::sockets.begin(); it != RawIPSocket::sockets.end(); ++it) {
			if((*it)->protocol() == ask::Socket::PROTO_ANY || (*it)->protocol() == proto)
				(*it)->push(ask::Socket::Addr(),packet,ETHER_HEAD_SIZE);
		}
	...
	uint8_t protocol;
	uint16_t checksum;
	ask::Net::IPv4Addr src;
	ask::Net::IPv4Addr dst;
	T payload;
} A_PACKED;
