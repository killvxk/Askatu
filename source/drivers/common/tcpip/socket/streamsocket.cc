...
int StreamSocket::connect(const ask::Socket::Addr *sa,msgid_t mid) {
...
int StreamSocket::bind(const ask::Socket::Addr *sa) {
	if(_localPort != 0)
		return -EINVAL;
...
int StreamSocket::accept(msgid_t mid,int nfd,ask::ClientDevice<Socket> *dev) {
...
ssize_t StreamSocket::sendto(msgid_t mid,const ask::Socket::Addr *,const void *data,size_t size) {
...
void StreamSocket::push(const ask::Socket::Addr &,const Packet &pkt,size_t) {
	...
	// validate checksum
	uint16_t checksum = ask::Net::ipv4PayloadChecksum(ip->src,ip->dst,TCP::IP_PROTO,
	...
	switch(_state) {
		case STATE_LISTEN: {
			if(tcp->ctrlFlags == TCP::FL_SYN) {
				...
				syn.src.family = ask::Socket::AF_INET;
		...
		case STATE_SYN_RECEIVED: {
			if(ackNo > _ctrlpkt.seqNo && (tcp->ctrlFlags & TCP::FL_ACK)) {
				...
				ask::IPCStream is(_pending.d.accept.fd,buffer,sizeof(buffer),_pending.mid);
...
int StreamSocket::forkSocket(int nfd,msgid_t mid,ask::ClientDevice<Socket> *dev,SynPacket &syn,
		CircularBuf::seq_type seqNo) {
	StreamSocket *s = new StreamSocket(nfd,ask::Socket::PROTO_TCP);
	s->_mss = syn.mss;
