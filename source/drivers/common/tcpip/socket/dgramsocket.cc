...
int DGramSocket::bind(const ask::Socket::Addr *sa) {
...
ssize_t DGramSocket::sendto(msgid_t,const ask::Socket::Addr *sa,const void *buffer,size_t size) {
	if(sa == NULL)
		return -EINVAL;
	...
	return UDP::send(ask::Net::IPv4Addr(sa->d.ipv4.addr),_localPort,sa->d.ipv4.port,buffer,size);
}
