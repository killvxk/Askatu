...
public:
	...
	virtual int connect(const ask::Socket::Addr *sa,msgid_t mid);
	virtual int bind(const ask::Socket::Addr *sa);
	virtual int listen();
	virtual int accept(msgid_t mid,int nfd,ask::ClientDevice<Socket> *dev);
	...
	virtual void push(const ask::Socket::Addr &sa,const Packet &pkt,size_t offset);
	virtual int abort();
	virtual void disconnect();

	ask::port_t localPort() const {
		return _localPort;
	}
	ask::Net::IPv4Addr remoteIP() const {
		return ask::Net::IPv4Addr(_remoteAddr.d.ipv4.addr);
	}
	ask::port_t remotePort() const {
...
private:
	...
	int forkSocket(int nfd,msgid_t mid,ask::ClientDevice<Socket> *dev,SynPacket &syn,
		CircularBuf::seq_type seqNo);
	...
	void replyPending(T result) {
		if(_pending.count > 0) {
			ulong buffer[1];
			ask::IPCStream is(fd(),buffer,sizeof(buffer),_pending.mid);
			is << ask::ReplyData(&result,sizeof(T));
			_pending.count = 0;
		}
	}
