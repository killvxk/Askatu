...
static void list_fillDev(ask::PCI::Device *dev);
static void list_fillBar(ask::PCI::Device *dev,size_t i);

static std::vector<ask::PCI::Device*> devices;
...
ask::PCI::Device *list_getByClass(uchar baseClass,uchar subClass,int no) {
...
ask::PCI::Device *list_getById(uchar bus,uchar dev,uchar func) {
	for(auto d = devices.begin(); d != devices.end(); ++d) {
...
ask::PCI::Device *list_get(size_t i) {
	if(i >= devices.size())
		return NULL;
	return devices[i];
}
	...
	for(bus = 0; bus < BUS_COUNT; bus++) {
		for(dev = 0; dev < DEV_COUNT; dev++) {
			for(func = 0; func < FUNC_COUNT; func++) {
				ask::PCI::Device *device = (ask::PCI::Device*)malloc(sizeof(ask::PCI::Device));
				if(!device)
	...
	if(bar->size == 0 || bar->size == 0xFFFFFFFF)
		...
		if((bar->size & BAR_ADDR_SPACE) == BAR_ADDR_SPACE_MEM) {
			switch(BAR_MEM_TYPE(barValue)) {
				case 0x00:
					bar->flags |= ask::PCI::Bar::BAR_MEM_32;
					break;
				case 0x02:
					bar->flags |= ask::PCI::Bar::BAR_MEM_64;
					break;
			}
			if(BAR_MEM_PREFETCHABLE(barValue))
				bar->flags |= ask::PCI::Bar::BAR_MEM_PREFETCH;
