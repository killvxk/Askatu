...
E1000::E1000(ask::PCI &pci,const ask::PCI::Device &nic)
	...
	// map MMIO region
	for(size_t i = 0; i < ARRAY_SIZE(ask::PCI::Device::bars); ++i) {
		if(nic.bars[i].addr && nic.bars[i].type == ask::PCI::Bar::BAR_MEM) {
	...
	// clear daskriptors
	memset(_bufs->rxDasks,0,sizeof(_bufs->rxDasks));
	memset(_bufs->txDasks,0,sizeof(_bufs->txDasks));

	// reset card
	reset();

	// create the IRQ sem here to ensure that we've registered it if the first interrupt arrives
	if(pci.hasCap(nic.bus,nic.dev,nic.func,ask::PCI::CAP_MSI)) {
...
ask::NIC::MAC E1000::readMAC() {
	uint8_t bytes[ask::NIC::MAC::LEN];
	...
	if(macaddr != ask::NIC::MAC::broadcast() && macaddr.value() != 0)
		return macaddr;

	DBG1("Reading MAC from EEPROM");
	readEEPROM(bytes,sizeof(bytes));
	return ask::NIC::MAC(bytes);
}
...
void E1000::reset() {
	...
	writeReg(REG_RDBAL,reinterpret_cast<uintptr_t>(_bufsPhys->rxDasks + 0));
	writeReg(REG_RDLEN,RX_BUF_COUNT * sizeof(RxDask));
	writeReg(REG_RDH,0);
	...
	writeReg(REG_TDBAL,reinterpret_cast<uintptr_t>(_bufsPhys->txDasks + 0));
	writeReg(REG_TDLEN,TX_BUF_COUNT * sizeof(TxDask));
	writeReg(REG_TDH,0);
	writeReg(REG_TDT,0);
	writeReg(REG_TIDV,0);
	writeReg(REG_TADV,0);

	// setup rx daskriptors
	for(size_t i = 0; i < RX_BUF_COUNT; i++) {
		_bufs->rxDasks[i].length = RX_BUF_SIZE;
		_bufs->rxDasks[i].buffer = reinterpret_cast<uint64_t>(_bufsPhys->rxBuf + i * RX_BUF_SIZE);
	}
...
ssize_t E1000::send(const void *packet,size_t size) {
	...
	// setup daskriptor
	_bufs->txDasks[cur].cmd = TX_CMD_EOP | TX_CMD_IFCS;
	_bufs->txDasks[cur].length = size;
	_bufs->txDasks[cur].buffer = reinterpret_cast<uint64_t>(phys);
	_bufs->txDasks[cur].status = 0;
	asm volatile ("" : : : "memory");
...
void E1000::receive() {
	...
	while(_curRxBuf != head) {
		RxDask *dask = _bufs->rxDasks + _curRxBuf;

		if(~dask->status & RDS_DONE)
			break;

		DBG2("RX %u: %#08Lx..%#08Lx st=%#02x err=%#02x",
			_curRxBuf,dask->buffer,dask->buffer + dask->length,dask->status,dask->error);

		// read data into packet
		size_t size = dask->length;
