...
class E1000 : public ask::NICDriver {
	friend class EEPROM;
	...
	enum {
		...
		REG_RDBAL			= 0x2800,		/* register daskriptor base address low */
		REG_RDBAH			= 0x2804,		/* register daskriptor base address high */
		REG_RDLEN			= 0x2808,		/* register daskriptor length */
		REG_RDH				= 0x2810,		/* register daskriptor head */
		REG_RDT				= 0x2818,		/* register daskriptor tail */

		REG_RDTR			= 0x2820,		/* receive delay timer register */
		REG_RDCTL			= 0x2828,		/* transmit daskriptor control */
		REG_RADV			= 0x282c,		/* receive absolute interrupt delay timer */

		REG_TDBAL			= 0x3800,		/* transmit daskriptor base address low */
		REG_TDBAH			= 0x3804,		/* transmit daskriptor base address high */
		REG_TDLEN			= 0x3808,		/* transmit daskriptor length */
		REG_TDH				= 0x3810,		/* transmit daskriptor head */
		REG_TDT				= 0x3818,		/* transmit daskriptor tail */

		REG_TIDV			= 0x3820,		/* transmit interrupt delay value */
		REG_TDCTL			= 0x3828,		/* transmit daskriptor control */
	...
	enum {
		ICR_TXDW			= 1 << 0,		/* transmit daskriptor done */
	...
	enum {
		RDS_DONE			= 1 << 0,		/* receive daskriptor status; indicates that the HW has
											 * finished the daskriptor */
	};
	...
	struct Buffers {
		RxDask rxDasks[RX_BUF_COUNT] A_ALIGNED(16);
		TxDask txDasks[TX_BUF_COUNT] A_ALIGNED(16);
		uint8_t rxBuf[RX_BUF_COUNT * RX_BUF_SIZE];
		uint8_t txBuf[TX_BUF_COUNT * TX_BUF_SIZE];
	};

public:
	explicit E1000(ask::PCI &pci,const ask::PCI::Device &nic);
