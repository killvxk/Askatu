...
/* our state */
uint8_t *VGA::screen;
std::vector<ask::Screen::Mode> VGA::modes;
...
void VGA::ScreenDevice::setScreenMode(ScreenClient *c,const char *shm,Screen::Mode *mode,int type,bool sw) {
	if(type != ask::Screen::MODE_TYPE_TUI)
		throw ask::default_error("Invalid mode type");
...
void VGA::ScreenDevice::updateScreen(ScreenClient *c,gpos_t x,gpos_t y,gsize_t width,gsize_t height) {
	if(!c->mode || !c->fb)
		throw ask::default_error("No mode set");
...
void VGA::init() {
	...
	modes.push_back(((Screen::Mode){
		0x0001,40,25,0,0,4,0,0,0,0,0,0,VGA_ADDR,0,0,ask::Screen::MODE_TEXT,ask::Screen::MODE_TYPE_TUI
	}));
	modes.push_back(((Screen::Mode){
		0x0003,80,25,0,0,4,0,0,0,0,0,0,VGA_ADDR,0,0,ask::Screen::MODE_TEXT,ask::Screen::MODE_TYPE_TUI
	}));
}
