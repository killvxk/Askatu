...
VESAGUI::VESAGUI()
		: _cursorCopy(), _lastX(), _lastY(),_curCursor(ask::Screen::CURSOR_DEFAULT), _cursor() {
...
void VESAGUI::setCursor(VESAScreen *scr,void *shmem,int newCurX,int newCurY,int newCursor) {
	if(newCursor == ask::Screen::CURSOR_RESIZE_L || newCursor == ask::Screen::CURSOR_RESIZE_BL)
...
void VESAGUI::update(VESAScreen *scr,void *shmem,gpos_t x,gpos_t y,gsize_t width,gsize_t height) {
	ask::Screen::Mode *minfo = scr->mode;
	gpos_t y1,y2;
