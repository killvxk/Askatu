...
VESAGUI *VESA::gui;
VESATUI *VESA::tui;
std::vector<ask::Screen::Mode> VESA::modes;

void VESA::ScreenDevice::setScreenMode(Client *c,const char *shm,Screen::Mode *mode,int type,bool sw) {
	assert(type == ask::Screen::MODE_TYPE_TUI || type == ask::Screen::MODE_TYPE_GUI);
...
void VESA::ScreenDevice::setScreenCursor(Client *c,gpos_t x,gpos_t y,int cursor) {
	if(c->screen) {
		if(c->type() == ask::Screen::MODE_TYPE_TUI)
...
void VESA::init() {
	...
	for(auto m = VBE::begin(); m != VBE::end(); ++m) {
		if((m->second->modeAttributes & mask) == mask && m->second->bitsPerPixel >= 16 &&
			...
			mode.mode = ask::Screen::MODE_GRAPHICAL;
			mode.type = ask::Screen::MODE_TYPE_TUI | ask::Screen::MODE_TYPE_GUI;
