...
static void printDevice(ask::PCI &pci,const ask::PCI::Device *device,int verbose) {
	...
	if(verbose) {
		...
		if(device->type == ask::PCI::GENERIC) {
			size_t i;
			if(device->irq)
				printf("        IRQ: %u\n",device->irq);
			for(i = 0; i < 6; i++) {
				if(device->bars[i].addr) {
					if(device->bars[i].type == ask::PCI::Bar::BAR_MEM) {
						printf("        Memory at %p (%d-bit, %s) [size=%zuK]\n",
								(void*)device->bars[i].addr,
								(device->bars[i].flags & ask::PCI::Bar::BAR_MEM_32) ? 32 : 64,
								(device->bars[i].flags & ask::PCI::Bar::BAR_MEM_PREFETCH)
			...
			printf("        Status: ");
			if(status & ask::PCI::ST_IRQ)
				printf("IRQ ");
			if(status & ask::PCI::ST_CAPS)
				printf("CAPS ");
			printf("\n");
...
int main(int argc,const char *argv[]) {
	...
	for(size_t i = 0; i < count; ++i) {
		try {
			ask::PCI::Device dev = pci.getByIndex(i);
