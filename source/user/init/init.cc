...
class InitDevice : public ask::Device {
public:
	explicit InitDevice(const char *path,mode_t mode) : ask::Device(path,mode,DEV_TYPE_SERVICE,0) {
	...
	void iamalive(ask::IPCStream &is) {
		pid_t pid;
		is >> pid;
