...
bool Progress::connect() {
	...
	ask::Screen::Mode mode = _scr->findTextMode(VGA_COLS,VGA_ROWS);
	_fb = new ask::FrameBuffer(mode,"init-vga",ask::Screen::MODE_TYPE_TUI,0644);
	_scr->setMode(ask::Screen::MODE_TYPE_TUI,mode.id,"init-vga",true);
	paintBar();
	return true;
}
