...
class FileList : public gui::Panel {
	...
	public:
		explicit FileObject(FileList &list,const ask::file &file)
...
public:
	...
	void loadDir(const std::string &path) {
		...
		try {
			...
			vector<struct dirent> entries = ask::file(path).list_files(false);
			for(auto it = entries.begin(); it != entries.end(); ++it)
				files.push_back(ask::file(path,it->d_name));
			files.sort([] (const ask::file &a,const ask::file &b) {
	...
	void setList(const std::list<ask::file> &files) {
		removeAll();
		_files = files;
