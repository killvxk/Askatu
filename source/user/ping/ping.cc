...
static void sendEcho(Socket &sock,const Net::IPv4Addr &src,const Net::IPv4Addr &dest,
	...
	icmp->checksum = 0;
	icmp->checksum = ask::Net::ipv4Checksum(reinterpret_cast<uint16_t*>(icmp),sizeof(*icmp) + nbytes);
	...
	ip->dst = dest;
	ip->checksum = 0;
	ip->checksum = ask::Net::ipv4Checksum(reinterpret_cast<uint16_t*>(ip),sizeof(IPv4Header));
...
static int handleReply(IPv4Header *reply) {
	...
	recvTime = rdtsc();
	if(ask::Net::ipv4Checksum(reinterpret_cast<uint16_t*>(reply),sizeof(IPv4Header)) != 0) {
		std::cerr << "IP header has invalid checksum" << std::endl;
		return -1;
	}

	if(ask::Net::ipv4Checksum(reinterpret_cast<uint16_t*>(icmp),sizeof(*icmp) + nbytes) != 0) {
...
int main(int argc,char **argv) {
	...
	// parse params
	ask::cmdargs args(argc,argv,ask::cmdargs::MAX1_FREE);
	try {
