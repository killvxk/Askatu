...
int main(int argc,const char *argv[]) {
	...
	if(list) {
		std::vector<ask::Screen::Mode> modes = vterm.getModes();
		ask::Screen::Mode curMode = vterm.getMode();
		...
		for(auto it = modes.begin(); it != modes.end(); ++it) {
			printf("%c %5d: %3u x %3u cells, %4u x %4u pixels, %2ubpp, %s (%s,%s)\n",
					...
					it->mode == ask::Screen::MODE_TEXT ? "text     " : "graphical",
					(it->type & ask::Screen::MODE_TYPE_TUI) ? "tui" : "-",
					(it->type & ask::Screen::MODE_TYPE_GUI) ? "gui" : "-");
		}
	}
	else
		vterm.setMode(mode);
