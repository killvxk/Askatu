...
static void linkRem(ask::Net &net,int argc,char **argv);
...
static void linkAdd(ask::Net &net,int argc,char **argv) {
...
static void linkSet(ask::Net &net,int argc,char **argv) {
	if(argc != 5)
		usage(argv[0]);
...
static void linkUpDown(ask::Net &net,int argc,char **argv) {
	if(argc != 3)
		usage(argv[0]);
	...
	if(strcmp(argv[1],"up") == 0)
		net.linkConfig(argv[2],link->ip(),link->subnetMask(),ask::Net::UP);
	else if(strcmp(argv[1],"down") == 0)
		net.linkConfig(argv[2],link->ip(),link->subnetMask(),ask::Net::DOWN);
	else
		usage(argv[0]);
}
