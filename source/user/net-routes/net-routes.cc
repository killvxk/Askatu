...
static ask::Net::IPv4Addr strToIp(const char *str,bool allowDef = false) {
	ask::Net::IPv4Addr ip;
...
static void routeAdd(ask::Net &net,int argc,char **argv) {
	if(argc < 6)
		usage(argv[0]);
...
static void routeRem(ask::Net &net,int argc,char **argv) {
	if(argc < 3)
		usage(argv[0]);
...
static void routeUpDown(ask::Net &net,int argc,char **argv) {
	if(argc != 3)
		usage(argv[0]);

	if(strcmp(argv[1],"up") == 0)
		net.routeConfig(strToIp(argv[2]),ask::Net::UP);
	else
		net.routeConfig(strToIp(argv[2]),ask::Net::DOWN);
}
