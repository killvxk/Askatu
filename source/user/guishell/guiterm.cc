...
GUIVTermDevice::GUIVTermDevice(const char *path,mode_t mode,std::shared_ptr<ShellControl> sh,
		uint cols,uint rows)
	: ask::VTermDevice(path,mode,&_vt), _vt(), _mode(), _sh(sh),
	...
	// open speaker
	try {
		_vt.speaker = new ask::Speaker("/dev/speaker");
	}
	...
	_mode.cols = cols;
	_mode.rows = rows;
	_mode.type = ask::Screen::MODE_TYPE_TUI;
	_mode.mode = ask::Screen::MODE_GRAPHICAL;
...
void GUIVTermDevice::loop() {
	...
	while(!isStopped()) {
		...
		ask::IPCStream is(fd,buf,sizeof(buf),mid);
		handleMsg(mid,is);
		_sh->update();
	}
}

void GUIVTermDevice::getMode(ask::IPCStream &is) {
	prepareMode();
	is << 0 << _mode << ask::Reply();
}

void GUIVTermDevice::getModes(ask::IPCStream &is) {
	size_t n;
	is >> n;

	is << static_cast<ssize_t>(1) << ask::Reply();
	if(n) {
		prepareMode();
		is << ask::ReplyData(&_mode,sizeof(ask::Screen::Mode));
	}
}

void GUIVTermDevice::write(ask::IPCStream &is) {
	...
	ssize_t res = 0;
	if(data) {
		is >> ask::ReceiveData(data,r.count);
		data[r.count] = '\0';
		res = r.count;
