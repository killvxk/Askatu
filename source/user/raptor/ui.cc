...
void ui_init(uint cols,uint rows) {
	...
	do {
		...
		try {
			fb = new ask::FrameBuffer(mode,shmname,ask::Screen::MODE_TYPE_TUI,0644);
		}
		catch(...) {
		}
	}
	...
	/* set mode */
	ui->setMode(ask::Screen::MODE_TYPE_TUI,mode.id,shmname,true);
