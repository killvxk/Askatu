...
class FibDevice : public ask::Device {
public:
	explicit FibDevice(const char *path,mode_t mode)
			: ask::Device(path,mode,DEV_TYPE_SERVICE,DEV_OPEN), _next(0) {
...
static int clientthread(A_UNUSED void *arg) {
	int n = startFib;
	ask::IPCStream is("/dev/parallel");
	while(1) {
		int res;
		is << n << ask::SendReceive(0x1234) >> res;
		printf("[%d] fib(%d) = %d\n",gettid(),n,res);
		fflush(stdout);
		n++;
		sleep(200);
