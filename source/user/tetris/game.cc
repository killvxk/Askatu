...
void Game::start(int cols,int rows,int size,bool sound) {
	...
	if(sound && stat("/dev/speaker",&info) == 0) {
		try {
			_spk = new ask::Speaker("/dev/speaker");
		}
...
void Game::handleKey(const ask::UIEvents::Event &ev) {
	// automatically pause the game if our UI gets inactive
	if(ev.type == ask::UIEvents::Event::TYPE_UI_INACTIVE && _state != GAMEOVER) {
		_state = PAUSED;
		update(true);
		return;
	}

	if(ev.type != ask::UIEvents::Event::TYPE_KEYBOARD || (ev.d.keyb.modifier & STATE_BREAK))
		return;
