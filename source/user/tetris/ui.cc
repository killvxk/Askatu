...
UI::UI(int c,int r,int size)
	...
	for(int id = 1; _fb == NULL; ++id) {
		...
		try {
			_fb = new ask::FrameBuffer(_mode,shmname,ask::Screen::MODE_TYPE_TUI,0644);
		}
		catch(...) {
		}
	}

	_ui.setMode(ask::Screen::MODE_TYPE_TUI,_mode.id,shmname,true);
}
...
void UI::start() {
	...
	while(_run) {
		...
		switch(ev.type) {
			...
			case ask::UIEvents::Event::TYPE_UI_ACTIVE:
			case ask::UIEvents::Event::TYPE_UI_INACTIVE:
				Game::handleKey(ev);
				break;
