...
public:
	...
	/**
	 * Attaches this object to the given file-daskriptor
	 *
	 * @param fd
	 */
	 ...
	 *
	 * @param type the screen type (ask::Screen::MODE_TYPE_{TUI,GUI})
	...
	Mode findTextMode(uint cols,uint rows) {
		...
		if(modes.size() == 0)
			throw ask::default_error("No modes found");
