...
ssize_t VFSChannel::open(pid_t pid,const char *path,uint flags,int msgid,mode_t mode) {
	...
	/* send msg to driver */
	ib << ask::FileOpen::Request(flags,FSUser(p->getEUid(),p->getEGid(),p->getPid()),
		ask::CString(path),mode);
	if(ib.error()) {
		res = -EINVAL;
		goto error;
	}
...
ssize_t VFSChannel::read(pid_t pid,OpenFile *file,USER void *buffer,off_t offset,size_t count) {
	...
	ask::IPCBuf ib(ibuffer,sizeof(ibuffer));
	ssize_t res;
	...
	ib << ask::FileRead::Request(offset,count,useshm ? ((uintptr_t)buffer - (uintptr_t)shmem) : -1);
...
ssize_t VFSChannel::write(pid_t pid,OpenFile *file,USER const void *buffer,off_t offset,size_t count) {
	...
	ib << ask::FileWrite::Request(offset,count,useshm ? ((uintptr_t)buffer - (uintptr_t)shmem) : -1);
...
int VFSChannel::sharefile(pid_t pid,OpenFile *file,const char *path,void *cliaddr,size_t size) {
	...
	/* send msg to driver */
	ib << ask::FileShFile::Request(size,ask::CString(path));
	if(ib.error())
		return -EINVAL;
...
int VFSChannel::creatsibl(pid_t pid,OpenFile *file,VFSChannel *sibl,int arg) {
	...
	/* send msg to driver */
	ib << ask::FileCreatSibl::Request(sibl->fd,arg);
...
VFSChannel::Message *VFSChannel::getMsg(ask::SList<Message> *list,msgid_t mid,ushort flags) {
...
void VFSChannel::print(OStream &os) const {
	const ask::SList<Message> *lists[] = {&sendList,&recvList};
