...
ask::SListItem *NodeAllocator::freelist = NULL;
...
void *NodeAllocator::allocate() {
	...
	if(EXPECT_FALSE(freelist == NULL)) {
		...
		for(size_t i = oldCount; i < newCount; i++) {
			ask::SListItem *n = (ask::SListItem*)nodeArray.getObj(i);
			n->next(freelist);
			freelist = n;
		}
	}

	ask::SListItem *n = freelist;
...
void NodeAllocator::free(void *ptr) {
	ask::SListItem *n = (ask::SListItem*)ptr;
