...
bool TimerBase::intrpt() {
	...
	/* if a process has been waked up or the time-slice is over, raskhedule */
	res = false;
	if(foundThread || (perCPU[cpu].elapsedMsecs - perCPU[cpu].lastRaskhed) >= TIMESLICE) {
		perCPU[cpu].lastRaskhed = perCPU[cpu].elapsedMsecs;
		res = true;
	}
	return res;
}
