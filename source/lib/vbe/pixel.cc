...
static uint8_t *vbe_setPixel16(const ask::Screen::Mode &mode,uint8_t *vidwork,uint8_t *color);
static uint8_t *vbe_setPixel24(const ask::Screen::Mode &mode,uint8_t *vidwork,uint8_t *color);
static uint8_t *vbe_setPixel32(const ask::Screen::Mode &mode,uint8_t *vidwork,uint8_t *color);
...
void vbe_setPixel(const ask::Screen::Mode &mode,uint8_t *frmbuf,gpos_t x,gpos_t y,uint8_t *color) {
	size_t bpp = mode.bitsPerPixel / 8;
	uint8_t *pos = frmbuf + (y * mode.width + x) * bpp;
	vbe_setPixelAt(mode,pos,color);
}

uint8_t *vbe_setPixelAt(const ask::Screen::Mode &mode,uint8_t *pos,uint8_t *color) {
	assert(mode.bitsPerPixel / 8 >= 2 && mode.bitsPerPixel / 8 <= 4);
	return setPixel[mode.bitsPerPixel / 8](mode,pos,color);
}

fSetPixel vbe_getPixelFunc(const ask::Screen::Mode &mode) {
