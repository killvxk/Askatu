...
GZipHeader GZipHeader::read(FILE *f) {
	...
	if(!h.isGZip())
		throw ask::default_error("No GZip file");
	if(h.method != MDEFLATE)
		throw ask::default_error("Unknown compression method");
	...
	if(h.flags & FHCRC) {
		...
		if(crc16 != (checksum & 0x0000ffff))
			throw ask::default_error("GZip has invalid checksum");
	}
	return h;
}
...
void GZipHeader::write(FILE *f) {
	...
	try {
		if(fwrite(this,1,10,f) != 10)
			throw ask::default_error("Unable to write header");

		if(flags & FNAME) {
			assert(filename != NULL);
			size_t len = strlen(filename) + 1;
			if(fwrite(filename,1,len,f) != len)
				throw ask::default_error("Unable to write filename");
		}
		...
		if(flags & FCOMMENT) {
			...
			if(fwrite(comment,1,len,f) != len)
				throw ask::default_error("Unable to write comment");
		}
		...
		if(flags & FHCRC) {
			...
			if(fwrite(&hcrc,1,2,f) != 2)
				throw ask::default_error("Unable to write header CRC");
		}
