...
static ask::VTerm vterm(STDOUT_FILENO);
...
int shell_readLine(char *buffer,size_t max) {
	...
	vterm.setFlag(ask::VTerm::FL_NAVI,true);
	vterm.setFlag(ask::VTerm::FL_READLINE,false);
	vterm.setFlag(ask::VTerm::FL_ECHO,true);
	...
	*buffer = '\0';
	while(o < max) {
		...
		if(ibpos >= iblen) {
			...
			/* EOF? */
			if(iblen <= 0) {
				vterm.setFlag(ask::VTerm::FL_READLINE,true);
				return iblen;
			}
		}

		/* read from inbuf to askbuf */
		if(inbuf[ibpos] == '\033')
			ibpos++;
		while(i < MAX_ESCC_LENGTH && ibpos < iblen) {
			askbuf[i++] = inbuf[ibpos];
			if(inbuf[ibpos++] == ']')
				break;
		}
		askbuf[i] = '\0';
		...
		/* parse askatu code */
		const char *askbufptr = askbuf;
		int cmd = escc_get(&askbufptr,&n1,&n2,&n3);
		...
		/* ask code is complete, so start at the beginning next time */
		i = 0;
		/* skip other askatu-codes or incomplete ones */
