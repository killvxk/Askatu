...
static bool vtout_handleAskatu(sVTerm *vt,char **str);

void vtout_puts(sVTerm *vt,char *str,size_t len,bool resetRead) {
	size_t oldRow;
	char c,*start = str;

	/* are we waiting to finish an askatu-code? */
	if(vt->escapePos >= 0) {
		size_t oldLen = vt->escapePos;
		char *askPtr = vt->escapeBuf;
		...
		/* try it again */
		if(!vtout_handleAskatu(vt,&askPtr)) {
		...
		/* skip askatu-code */
		str += (askPtr - vt->escapeBuf) - oldLen;
		vt->escapePos = -1;
	}
	...
	while((c = *str)) {
		if(c == '\033') {
			str++;
			/* if the askatu-code is incomplete, store what we have so far and wait for
			 * further input */
			size_t oldEscRow = vt->row;
			if(!vtout_handleAskatu(vt,&str)) {
