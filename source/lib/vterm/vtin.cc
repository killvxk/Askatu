...
void vtin_handleKey(sVTerm *vt,uchar keycode,uchar modifier,char c) {
	...
	/* send askatu-code when we're not in readline-mode */
	if(!vt->readLine) {
		char askatu[SSTRLEN("\033[kc;123;123;15]") + 1];
		/* we want to treat the character as unsigned here and extend it to 32bit */
		uint code = *(uchar*)&c;
		snprintf(askatu,sizeof(askatu),"\033[kc;%u;%u;%u]",code,keycode,modifier);
		vt->inbuf->writen(askatu,strlen(askatu));
	}
	if(!(modifier & STATE_BREAK)) {
