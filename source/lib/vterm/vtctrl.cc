...
bool vtctrl_init(sVTerm *vt,ask::Screen::Mode *mode) {
	/* init state */
	...
	vt->inbufEOF = false;
	vt->inbuf = new ask::RingBuffer<char>(INPUT_BUF_SIZE,ask::RB_OVERWRITE);
...
int vtctrl_control(sVTerm *vt,uint cmd,int arg1,int arg2) {
	...
	switch(cmd) {
		...
		case MSG_VT_GETFLAG: {
			switch(arg1) {
				case ask::VTerm::FL_ECHO:
					res = vt->echo;
					break;
				case ask::VTerm::FL_NAVI:
					res = vt->navigation;
					break;
				case ask::VTerm::FL_READLINE:
					res = vt->readLine;
					break;
				default:
					res = -EINVAL;
