...
ask::Net::IPv4Addr DNS::_nameserver;
...
ask::Net::IPv4Addr DNS::getHost(const char *name,uint timeout) {
	if(isIPAddress(name)) {
		ask::Net::IPv4Addr addr;
...
ask::Net::IPv4Addr DNS::resolve(const char *name,uint timeout) {
	...
	// create socket
	ask::Socket sock("/dev/socket",ask::Socket::SOCK_DGRAM,ask::Socket::PROTO_UDP);
	...
	for(int i = 0; i < answers; ++i) {
		...
		if(be16tocpu(ans->type) == TYPE_A && be16tocpu(ans->length) == ask::Net::IPv4Addr::LEN)
			return ask::Net::IPv4Addr(data + sizeof(DNSAnswer));
	}
