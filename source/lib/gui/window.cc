	...
	void Window::onMousePressed(const MouseEvent &e) {
		if(_style == DEFAULT) {
			if(_header && e.getPos().y < (gpos_t)_header->getSize().height)
				_inTitle = true;
			else if(e.getPos().y >= (gpos_t)(getSize().height - ask::Screen::CURSOR_RESIZE_WIDTH))
				_inResizeBottom = true;
			if(!_header || e.getPos().y >= (gpos_t)_header->getSize().height) {
				if(e.getPos().x < (gpos_t)ask::Screen::CURSOR_RESIZE_WIDTH)
					_inResizeLeft = true;
				else if(e.getPos().x >= (gpos_t)(getSize().width - ask::Screen::CURSOR_RESIZE_WIDTH))
					_inResizeRight = true;
			}
