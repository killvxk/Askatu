...
namespace gui {
	...
	Application::Application(const char *winmng)
		...
		_winEv.subscribe(ask::WinMngEvents::Event::TYPE_CREATED);
		_winEv.subscribe(ask::WinMngEvents::Event::TYPE_ACTIVE);
		_winEv.subscribe(ask::WinMngEvents::Event::TYPE_DESTROYED);
	}
	...
	void Application::handleEvent(const ask::WinMngEvents::Event &ev) {
		switch(ev.type) {
			case ask::WinMngEvents::Event::TYPE_RESIZE: {
			...
			case ask::WinMngEvents::Event::TYPE_MOUSE:
			...
			case ask::WinMngEvents::Event::TYPE_KEYBOARD: {
				Window *w = getWindowById(ev.wid);
				if(w) {
			...
			case ask::WinMngEvents::Event::TYPE_SET_ACTIVE: {
				Window *w = getWindowById(ev.wid);
				if(w) {
			...
			case ask::WinMngEvents::Event::TYPE_CREATED:
				_created.send(ev.wid,ev.d.created.title);
				break;

			case ask::WinMngEvents::Event::TYPE_ACTIVE:
				_activated.send(ev.wid);
				break;

			case ask::WinMngEvents::Event::TYPE_DESTROYED:
				_destroyed.send(ev.wid);
				break;

			case ask::WinMngEvents::Event::TYPE_RESET: {
				Window *w = getWindowById(ev.wid);
